...
import remix.myplayer.util.OtherUtil
...
class Tag(context: Context, song: Song?) : ContextWrapper(context) {
    ...
    fun edit() {
        ...
        editDialog.customView?.let { root ->
            ...
            // TODO: 2020-9-3 增加长按获取文件名
            root.song_layout?.setOnLongClickListener {
                ToastUtil.show(this, "长按事件被触发$song")
                //Song{id=575041, title='(?)', displayName='商业的本质', album='商业的本质', albumId=305, artist='<unknown>',
                // duration=2670156, realTime='44:30', url='/storage/emulated/0/03_1音频文件/商业的本质/商业的本质.mp3', size=10414785, year=2016}
                root.song_layout.editText?.setText(OtherUtil.getFileName(song.url))
                LogUtils.i("音乐信息 $song")
                true
            }
