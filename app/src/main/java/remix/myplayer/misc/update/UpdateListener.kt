...
class UpdateListener(activity: Context) : Listener {
        ...
        when (code) {
            UpdateStatus.Yes -> {
                val builder = Theme.getBaseDialog(ref.get() ?: return)
                        ...
                        .onPositive { _, _ ->
                            ref.get()?.startService(Intent(ref.get()
                                    ?: return@onPositive, DownloadService::class.java)
                ...
                if (!isForce(release)) {
                    builder
                            ...
                            .onNegative { _, _ ->
                                SPUtil.putValue(ref.get()
                                        ?: return@onNegative, SPUtil.UPDATE_KEY.NAME, SPUtil.UPDATE_KEY.IGNORE_FOREVER, true)
                            }
                            .neutralText(R.string.ignore_this_version)
                            .neutralColorAttr(R.attr.text_color_primary)
                            .onNeutral { _, _ ->
                                SPUtil.putValue(ref.get()
                                        ?: return@onNeutral, SPUtil.UPDATE_KEY.NAME, UpdateAgent.getOnlineVersionCode(release).toString(), true)
                            }
                } else {
    ...
    override fun onUpdateError(throwable: Throwable) {
        LogUtils.v("onUpdateError: $throwable")
    }
