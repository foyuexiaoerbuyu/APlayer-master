...
class MusicService : BaseService(), Playback, MusicEventCallback,
    ...
    var playModel: Int = MODE_LOOP
        set(newPlayModel) {
            LogUtils.v("修改播放模式: $newPlayModel")
    ...
    private var showDesktopLyric = false
        set(value) {
            LogUtils.v("设置桌面歌词开关, old: $field new: $value")
            field = value
        }
    ...
    val progress: Int
        get() {
            try {
                ...
                LogUtils.v("getProgress() %s", e.toString())
            }
    ...
    override fun onTaskRemoved(rootIntent: Intent) {
        ...
        LogUtils.v(TAG_LIFECYCLE, "onTaskRemoved")
//    unInit()
//    stopSelf()
//    System.exit(0)
    }
    ...
    override fun onDestroy() {
        LogUtils.v(TAG_LIFECYCLE, "onDestroy")
        super.onDestroy()
        stop = true
        unInit()
    }
    ...
    override fun onCreate() {
        super.onCreate()
        LogUtils.v(TAG_LIFECYCLE, "onCreate")
        service = this
        setUp()
    }
    ...
    override fun onStartCommand(commandIntent: Intent?, flags: Int, startId: Int): Int {
        ...
        LogUtils.v(TAG_LIFECYCLE, "onStartCommand, control: $control action: $action flags: $flags startId: $startId")
        stop = false
    ...
    override fun onSharedPreferenceChanged(sp: SharedPreferences, key: String) {
        LogUtils.v("onSharedPreferenceChanged, key: $key")
        when (key) {
            //通知栏背景色
    ...
    private fun setUpSession() {
        ...
        mediaSession.setCallback(object : MediaSessionCompat.Callback() {
            ...
            override fun onSkipToNext() {
                LogUtils.v("onSkipToNext")
                playNext()
            }

            override fun onSkipToPrevious() {
                LogUtils.v("onSkipToPrevious")
                playPrevious()
            }
    ...
    private fun setUpPlayer() {
        ...
        mediaPlayer.setOnPreparedListener { mp ->
            LogUtils.v("准备完成:%s" + firstPrepared)
...
//    }
    ...
    override fun pause(updateMediasessionOnly: Boolean) {
        LogUtils.v("pause: $updateMediasessionOnly")
    ...
    override fun playSelectSong(position: Int) {
        LogUtils.v("playSelectSong, $position")
...
//    }
    ...
    inner class WidgetReceiver : BroadcastReceiver() {
        ...
        override fun onReceive(context: Context, intent: Intent) {
            ...
            LogUtils.v("name: $name appIds: $appIds")
            when (name) {
    ...
    private fun handleStartCommandIntent(commandIntent: Intent?, action: String?) {
        LogUtils.v("handleStartCommandIntent")
        if (action == null) {
            return
        }
        firstPrepared = false
...
//                            LogUtils.v(it)
    ...
    private fun prepare(path: String, requestFocus: Boolean = true) {
        tryLaunch(
                block = {
                    ...
                    prepared = true
                    LogUtils.v("prepare finish: $path")
                },
                catch = {
    ...
    private fun updateDesktopLyric(force: Boolean) {
        LogUtils.v("updateDesktopLyric, showDesktopLyric: $showDesktopLyric")
        if (!showDesktopLyric) {
            return
        }
    ...
    private inner class LyricTask : TimerTask() {
        ...
        override fun run() {
            if (!showDesktopLyric) {
                LogUtils.v(TAG_DESKTOP_LYRIC, "已经关闭桌面歌词")
                uiHandler.sendEmptyMessage(REMOVE_DESKTOP_LRC)
                return
            }
            if (songInLyricTask != playQueue.song) {
                ...
                LogUtils.v(TAG_DESKTOP_LYRIC, "重新获取歌词内容, id: ${songInLyricTask.id}")
                return
            }
            if (force) {
                force = false
                lyricFetcher.updateLyricRows(songInLyricTask)
                LogUtils.v(TAG_DESKTOP_LYRIC, "强制重新获取歌词, id: ${songInLyricTask.id}")
                return
            }
            //判断权限
            ...
            if (isAppOnForeground()) {
                ...
                if (!isDesktopLyricShowing) {
                    ...
                    LogUtils.v(TAG_DESKTOP_LYRIC, "请求创建桌面歌词")
...
//      uiHandler.sendEmptyMessage(REMOVE_DESKTOP_LRC)
            LogUtils.v(TAG_DESKTOP_LYRIC, "停止更新桌面歌词")
            return super.cancel()
        }
    ...
    private fun createDesktopLyric() {
        ...
        LogUtils.v(TAG_DESKTOP_LYRIC, "创建桌面歌词")
    }
    ...
    private fun removeDesktopLyric() {
        if (desktopLyricView != null) {
            LogUtils.v(TAG_DESKTOP_LYRIC, "移除桌面歌词")
...
//          musicService.handlePlayStateChange()
    ...
    private inner class ScreenReceiver : BroadcastReceiver() {
        ...
        override fun onReceive(context: Context, intent: Intent) {
            val action = intent.action
            LogUtils.v("ScreenReceiver", action)
